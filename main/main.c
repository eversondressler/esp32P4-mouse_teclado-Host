...
#include <string.h>
#include <unistd.h>
#include "freertos/FreeRTOS.h" // IWYU pragma: keep (deve vir antes dos outros headers do FreeRTOS)
...
{
	if(((modifier & HID_LEFT_ALT) == HID_LEFT_ALT) || ((modifier & HID_RIGHT_ALT) == HID_RIGHT_ALT)) 
	{
		return true;
	}
...
void hid_host_interface_callback(hid_host_device_handle_t hid_device_handle, const hid_host_interface_event_t event, void *arg)
{
	...
	switch(event) 
	{
		...
		default:
			ESP_LOGW(TAG, "HID Device, protocol '%s' Unhandled event: %d (possibly suspend/resume)", hid_proto_name_str[dev_params.proto], event);
			break;
	}
}
...
static void usb_lib_task(void *arg)
{
	const usb_host_config_t host_config = 
	{
		...
		// Porta raiz ligada manualmente, depois de forçar Full Speed (abaixo)
		.root_port_unpowered = true,
	};

	ESP_ERROR_CHECK(usb_host_install(&host_config));

	// O hub da placa (Waveshare ESP32-P4-Module-DEV-KIT) enumera em High Speed e a
	// biblioteca USB Host ainda não implementa Transaction Translator (TT), então
	// mouses/teclados FS/LS ligados ao hub são recusados. Forçando o controlador
	// USB-DWC HS a operar só em FS/LS, o hub também enumera em FS e os dispositivos
	// passam direto por ele, sem precisar de TT.
